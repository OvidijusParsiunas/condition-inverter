...
export class GetIfStatementPositionAtEdge {
  ...
  public static validateAndGetTokenIndex(
    ...
    if (tokenIndex > -1) {
      if (tokenIndex === 0 || tokenIndex === tokens.length - 1) {
        return GetIfStatementPositionAtEdge.validateEdge(editor, tokenIndex, lineNum, startChar, endChar, getIndex, delta);
      }
      return tokenIndex;
    }
    return -1;
  ...
  public static validateAndGetCharIndex(editor: TextEditor, line: number, startChar: number, endChar: number): number {
    ...
    const ifStatementIndex = GetIfStatementPositionAtEdge.validateAndGetTokenIndex(editor, tokens, line, startChar, endChar, false);
