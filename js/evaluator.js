...
module.exports = class Evaluator {
  ...
  dealWithStandaloneStatements(tokens, currentTokenIndex) {
    if (this.shouldBracketsBeRemoved) {
      ...
      this.conditionsToBeInverted.push({ start: this.startOfCurrentlyEvaluatedStatementIndex, revertBooleanLiteral: this.revertBooleanLiteral });
    ...
    if (this.isOperationWrappableInBrackets && !this.areBracketsAlreadyPresent) {
      ...
      this.conditionsToBeInverted.push({ brackets: true, start: this.startOfCurrentlyEvaluatedStatementIndex, end: endIndex });
    }
  }
    ...
    for (let i = 0; i < tokens.length; i += 1) {
      if (this.isCurrentlyEvaluatingIfStatement) {
        if (this.currentIfStatementCloseBracketIndex <= i) {
          ...
          if (tokens[i + 1] === '=') {
            this.conditionsToBeInverted.push({ start: i, hasFollowupEquals: true });
            i += 1;
          } else {
            this.conditionsToBeInverted.push({ start: i });
          }
        } else if (tokens[i] === '=') {
          this.logicalOperatorFound = true;
