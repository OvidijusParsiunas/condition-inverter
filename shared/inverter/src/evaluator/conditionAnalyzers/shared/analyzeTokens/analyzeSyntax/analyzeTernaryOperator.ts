...
export class AnalyzeTernaryOperator {
  ...
  private static movePastColonExpression(tokens: Tokens, colonIndex: number, evaluationState: EvaluationState): number {
    ...
    if (closingBracketIndex > -1 && closingBracketIndex < evaluationState.conditionSequenceEndIndex) return closingBracketIndex - 1;
  ...
  public static movePastTernaryOperator(tokens: Tokens, postQuestionMarkIndex: number, evaluationState: EvaluationState): number {
    const colonIndex = TraversalUtil.getIndexOfCurrentTernaryColon(tokens, postQuestionMarkIndex, 1);
    if (colonIndex > -1) {
      return AnalyzeTernaryOperator.movePastColonExpression(tokens, colonIndex, evaluationState);
    }
    // get the token before the last one - mostly used for partial highlight outside of statement
    return tokens.length - 2;
  }
