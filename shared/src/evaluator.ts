...
export default class Evaluator {
  ...
  dealWithStandaloneStatements(tokens, currentTokenIndex) {
    if (this.shouldBracketsBeRemoved) {
      const endIndex = TraversalUtils.getIndexOfLastBracketOfIfStatement(
        tokens,
        this.startOfCurrentlyEvaluatedStatementIndex - 1,
      );
  ...
  evaluate(tokens) {
    ...
    for (let i = 0; i < tokens.length; i += 1) {
      if (this.isCurrentlyEvaluatingIfStatement) {
        ...
        this.startOfCurrentlyEvaluatedStatementIndex = TraversalUtils.findNonSpaceCharacterIndexStartingFromIndex(
          tokens,
          bracketIndex + 1,
        );
